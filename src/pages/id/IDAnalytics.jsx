...
/* CJA-style freeform-table workspace for Indonesia acquisition analytics */
      ...
      <div className="max-w-[1200px] mx-auto space-y-5 animate-fadeIn">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="text-base font-semibold text-text-primary">Acquisition Campaign Performance</h2>
            <span className="text-[10px] bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded font-medium">Auto-refresh: 15 min</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-text-secondary">
            <span className="px-2.5 py-1 bg-bg-card border border-border rounded">Data view: <strong className="text-text-primary">ID Production</strong></span>
        ...
        <div className="grid grid-cols-[1fr_340px] gap-5">
          ...
          {/* LEFT — Freeform Table Panels */}
          <div className="space-y-5">

            {/* Panel 1: Acquisition Funnel — Freeform Table */}
            <div className="bg-bg-card rounded-lg border border-border overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-border bg-gray-50/60">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-text-secondary">— Acquisition Funnel</span>
                </div>
              ...
              {idMetrics.funnel.map((step, i) => {
                ...
                const prevValue = i > 0 ? idMetrics.funnel[i - 1].value : step.value;
                const dropoff = i > 0 ? prevValue - step.value : 0;
                const dropPct = i > 0 ? ((dropoff / prevValue) * 100).toFixed(1) : '—';
                return (
                  <div
                    key={i}
                    className={`grid grid-cols-[1fr_100px_80px_80px] gap-0 px-4 py-2.5 border-b border-border/50 items-center ${i === 0 ? 'bg-emerald-50/30' : 'hover:bg-gray-50/50'}`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-text-primary font-medium w-[130px] shrink-0">{step.stage}</span>
              ...
              <div className="px-4 py-2.5 bg-gray-50/80 flex items-center justify-between">
                <span className="text-[11px] font-semibold text-text-secondary">End-to-End Conversion</span>
                <span className="text-sm font-bold text-text-primary">{idMetrics.funnel[idMetrics.funnel.length - 1].pct}%</span>
              </div>
            </div>
              ...
              {idMetrics.channelAttribution.map((ch, i) => (
                <div key={i} className="grid grid-cols-[1fr_80px_80px] gap-0 px-4 py-2.5 border-b border-border/50 items-center hover:bg-gray-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-text-primary font-medium w-[100px] shrink-0">{ch.channel}</span>
                    <div className="flex-1 h-4 bg-gray-100 rounded-sm overflow-hidden">
                      <div
                        className="h-full rounded-sm"
                  ...
                  <span className="text-[11px] text-text-primary text-right tabular-nums">
                    {i === 0 ? 'IDR 18K' : i === 1 ? 'IDR 24K' : i === 2 ? 'IDR 31K' : 'IDR 12K'}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-bg-card rounded-lg border border-border p-4 space-y-3">
              ...
              {[
                { label: 'Ad Impressions', value: '50,000', sub: 'last 30d' },
                { label: 'Activated Accounts', value: '680', sub: '1.36% conv.' },
                { label: 'Cost per Acquisition', value: 'IDR 22K', sub: '−12% vs prior' },
                { label: 'Avg. Days to Activate', value: '8.7', sub: 'from first click' },
                { label: 'KYC Completion Rate', value: '49.7%', sub: '920 of 1,850' },
              ].map((m, i) => (
            ...
            <div className="bg-bg-card rounded-lg border border-border overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <p className="text-sm text-text-primary leading-relaxed">{idMetrics.aiInsight.finding}</p>
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-wider mb-1">Recommendation</p>
                  <p className="text-xs text-text-primary">{idMetrics.aiInsight.recommendation}</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                  <span className="text-xs font-medium text-green-700">{idMetrics.aiInsight.impact}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-bg-card rounded-lg border border-border p-3">
              ...
              <div className="space-y-1">
                {['People', 'Sessions', 'Events', 'Impressions', 'CTR', 'CPA', 'KYC Rate'].map((c) => (
